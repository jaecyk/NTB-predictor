...
def reset_defaults():
    ...
    st.session_state.pop("pred_features", None)
...
def load_models():
    ...
    for tenor, fname in MODEL_FILES.items():
        path = Path(fname)
        if path.exists():
            models[tenor] = joblib.load(path)
            available.append(tenor)
        else:
            missing.append((tenor, fname))

    return models, available, missing
...
def build_feature_table() -> pd.DataFrame:
    rows = []
    for tenor in TENORS:
        features = derive_tenor_features(tenor)
        rows.append({"tenor": f"{tenor}D", **features})
...
def validate_inputs() -> list[str]:
    ...
    if float(st.session_state["system_liquidity"]) < 0:
        issues.append("System liquidity cannot be negative.")
    if float(st.session_state["mpr"]) <= 0:
        issues.append("MPR should be above zero.")
    if float(st.session_state["inflation"]) <= 0:
        issues.append("Inflation should be above zero.")

    for tenor in TENORS:
        if float(st.session_state[f"offer_{tenor}"]) < 0:
            issues.append(f"{tenor}D offer amount cannot be negative.")
        if float(st.session_state[f"prev_offer_{tenor}"]) < 0:
            issues.append(f"{tenor}D previous offer cannot be negative.")
        if float(st.session_state[f"prev_bid_cover_{tenor}"]) < 0:
            issues.append(f"{tenor}D previous bid cover cannot be negative.")
        if float(st.session_state[f"sec_rate_{tenor}"]) <= 0:
            issues.append(f"{tenor}D secondary rate should be above zero.")
        if float(st.session_state[f"sec_rate_5d_ago_{tenor}"]) <= 0:
            issues.append(f"{tenor}D secondary rate 5D ago should be above zero.")
...
def predict_all(models: dict):
    ...
    for tenor in TENORS:
        features = derive_tenor_features(tenor)
        feature_rows.append({"tenor": f"{tenor}D", **features})

        if tenor not in models:
            predictions[tenor] = "Model file not available"
            continue
        ...
        try:
            ...
            predictions[tenor] = round(yhat, 4)
        except Exception as exc:
            predictions[tenor] = f"{type(exc).__name__}: {exc}"

    return predictions, pd.DataFrame(feature_rows)


def build_export_table(predictions: dict) -> pd.DataFrame:
    rows = []
    for tenor in TENORS:
        value = predictions.get(tenor)
        rows.append(
            {
                "auction_date": st.session_state["auction_date"],
                "tenor": f"{tenor}D",
                "predicted_stop_rate": value if isinstance(value, (float, int)) else None,
                "status": "ok" if isinstance(value, (float, int)) else str(value),
                "secondary_rate": float(st.session_state[f"sec_rate_{tenor}"]),
                "lag1_stop": float(st.session_state[f"lag1_stop_{tenor}"]),
                "offer_amount_bn": float(st.session_state[f"offer_{tenor}"]),
                "previous_bid_cover": float(st.session_state[f"prev_bid_cover_{tenor}"]),
            }
        )
    return pd.DataFrame(rows)
...
<div class="hero">
    <div class="hero-top">
        ...
        <div class="hero-badges">
            ...
            <span class="badge">v2 Preview</span>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("Use this panel for quick actions and diagnostics.")

    if st.button("Reset to default inputs", use_container_width=True):
        reset_defaults()
        st.rerun()

    st.markdown("---")
    st.markdown("**Environment Notes**")
    st.write(f"Expected scikit-learn version for saved models: `{EXPECTED_SKLEARN_VERSION}`")
    st.write(f"Available models: {', '.join([f'{t}D' for t in available_models]) if available_models else 'None'}")

    if missing_models:
        missing_text = ", ".join([f"{tenor}D ({fname})" for tenor, fname in missing_models])
        st.warning(f"Missing model files: {missing_text}")

# =========================================================
# MODEL STATUS MESSAGE
# =========================================================
if not available_models:
    st.error("No model files were found. Add the three pickle files to the project root to run predictions.")
else:
    if missing_models:
        missing_text = ", ".join([f"{tenor}D ({fname})" for tenor, fname in missing_models])
        st.warning(
            "Some model files are missing, but the app will still run for the available tenors. "
            f"Missing: {missing_text}"
        )
    else:
        st.success("All tenor model files are available.")
    ...
    with st.container(border=True):
        ...
        for tenor, tab in zip(TENORS, tabs):
            with tab:
                ...
                feature_preview = derive_tenor_features(tenor)
                p1, p2, p3 = st.columns(3)
                with p1:
                    st.metric("MA3 stop", f"{feature_preview['ma3_stop']:.2f}%")
                with p2:
                    st.metric("Momentum", f"{feature_preview['delta_stop_1']:+.2f}")
                with p3:
                    st.metric("Sec vs lag1", f"{feature_preview['sec_minus_lag1']:+.2f}")
...
with right:
    with st.container(border=True):
        ...
        can_predict = len(available_models) > 0 and len(issues) == 0

        if st.button(
            "Predict Stop Rates",
            type="primary",
            use_container_width=True,
            help="Run the available tenor models using the current pre-auction scenario.",
            disabled=not can_predict,
        ):
            predictions, pred_features = predict_all(models)
    ...
    if "predictions" in st.session_state:
        with st.container(border=True):
            ...
            for idx, tenor in enumerate(TENORS):
                value = st.session_state["predictions"][tenor]
                col = [c1, c2, c3][idx]

                if isinstance(value, (float, int)):
                    sec_rate = float(st.session_state[f"sec_rate_{tenor}"])
                    lag1 = float(st.session_state[f"lag1_stop_{tenor}"])
                    interpretation = interpret_result(value, sec_rate, lag1)

                    col.metric(f"{tenor}D Estimated Stop Rate", f"{value:.2f}%")
                    col.caption(interpretation)
                else:
                    col.error(f"{tenor}D prediction unavailable")
        ...
        with st.container(border=True):
            ...
            for tenor in TENORS:
                value = st.session_state["predictions"][tenor]
                if isinstance(value, (float, int)):
                    st.markdown(f"**{tenor}D:** {build_tenor_interpretation(tenor, float(value))}")
                else:
                    st.markdown(f"**{tenor}D:** Interpretation unavailable because no valid prediction was produced.")
        ...
        with st.container(border=True):
            ...
            export_df = build_export_table(st.session_state["predictions"])
            st.download_button(
                "Download predictions as CSV",
                export_df.to_csv(index=False).encode("utf-8"),
                file_name=f"ntb_stop_rate_predictions_{date.today().isoformat()}.csv",
                mime="text/csv",
                use_container_width=True,
            )
    ...
    with st.container(border=True):
        ...
        st.caption("For internal treasury decision support only. Use alongside market colour, demand context, and dealer judgement.")
